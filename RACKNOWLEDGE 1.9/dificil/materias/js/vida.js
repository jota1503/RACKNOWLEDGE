...
{
    // última identificación para nodos
    /** @type {number} */
    this.last_id = 0;

    // Tamaño del mapa hash.
     // Siempre una potencia de 2 menos 1
    this.hashmap_size = 0;


    // Tamaño cuando suceda el próximo GC
    this.max_load = 0;
    ...
    // configuración de reglas actual
    /** @type {number} */
    this.rule_b = 1 << 3;
    /** @type {number} */
    this.rule_s =  1 << 2 | 1 << 3;
...
LifeUniverse.prototype.restore_rewind_state = function()
{
    ...
    // asegúrese de reconstruir el mapa hash, en caso de que su tamaño cambie
...
LifeUniverse.prototype.set_bit = function(x, y, living)
{
    ...
    else
    {
        if(level > this.root.level) {
            // no es necesario eliminar píxeles fuera del universo
            return;
        }
    }
...
// Preservar el árbol, pero eliminar todo lo almacenado en caché.
...
// Devuelve falso si un nodo está en el hashmap
...
// insertar un nodo en el mapa hash
...
// crear o buscar un nodo de árbol dados sus hijos
...
// la función hash utilizada para el mapa hash
...
/*
 * dado un punto {x, y} o un objeto de límites {izquierda, arriba, abajo, derecha},
  * devolver el nivel de quadtree que se requiere para contener este punto
  */
LifeUniverse.prototype.get_level_from_bounds = function(bounds)
{
    // La raíz siempre debe ser al menos el nivel 3
    var max = 4,
...
{
    // Como partición de clasificación rápida: separe los valores de principio a fin mediante 
    // la máscara de bits en desplazamiento en la matriz text_field, devolviendo el medio
    var i = start,
        j = end,
        swap;
...
LifeUniverse.prototype.setup_field_recurse = function(start, end, field_x, field_y, level)
{
    ...
    var offset = 1 << level,
        // Aquí dividimos el campo de principio a fin en 4 partes:
        //    [Inicio, parte 2] -> nw
         // [Parte2, parte3] -> ne
         // [Parte3, parte4] -> sw
         // [Parte 4, fin] -> se
        //
        //  Primero dividimos [inicio, fin] en norte y sur dividiendo
         // por las coordenadas y. A continuación dividimos las dos mitades por la coordenada x.
...
LifeUniverse.prototype.level2_setup = function(start, end, field_x, field_y)
{
    ...
    for(var i = start; i <= end; i++)
    {
        ...
        // intercalar valores x e y de 2 bits
...
LifeUniverse.prototype.TreeNode = function(nw, ne, sw, se, id)
{
    ...
    // 2^(nivel - 2) generaciones en adelante
    this.quick_cache = null;

    // siguiente entrada en el mapa hash si este nodo ocupa la misma ranura
...
LifeUniverse.prototype.node_get_bit = function(node, x, y)
{
    ...
    if(node.level === 0)
    {
        // otro caso de nivel 0 se maneja arriba
        return true;
    }
...
LifeUniverse.prototype.node_get_boundary = function(node, left, top, find_mask, boundary)
{
    ...
    else
    {
        ...
        {
            // este cuadrado ya está dentro del límite encontrado
            return;
        }
